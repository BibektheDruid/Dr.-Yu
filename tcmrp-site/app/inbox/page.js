...
// Private viewer for contact-form submissions. Same pattern as intake-form.
// Visit /inbox?key=YOUR_INBOX_KEY (INBOX_KEY env var).
...
export default async function Inbox({ searchParams }) {
  ...
  if (!expected || key !== expected) {
    return (
      <main style={wrap}>
        <h1 style={{ color: '#1d6531' }}>Wellspring — Submissions</h1>
  ...
  return (
    <main style={wrap}>
      <h1 style={{ color: '#1d6531', marginBottom: 4 }}>Wellspring — Submissions</h1>
      ...
      {subs.map(({ meta, data }) => {
        ...
        return (
          ...
          >
            ...
            <details>
              <summary style={{ cursor: 'pointer', color: '#1d6531', fontWeight: 600 }}>All answers</summary>
